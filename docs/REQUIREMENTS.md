...
* [ ] Security model must clearly state what front.js **does not protect against** — (MISSING: add `docs/Security-Model.md`)
...
* [~] Rendering layer must be **replaceable** — (core currently imports and re-exports `uhtml` directly; creating an internal wrapper would improve swappability)
* [x] uhtml v4 is acceptable as default renderer (for now) — (used in `src/core/component.js`)
* [ ] Renderer must be wrapped behind an internal abstraction:

  * [ ] `html()` for templating
  * [ ] `render(container, template)`
* [ ] Core runtime must not depend on renderer internals — (currently depends on `uhtml` imports)
* [ ] Ability to swap renderer without breaking public API — (not currently proven)
...
* [x] Core public API remains small: (`src/index.js` exports `val`, `run`, `calc`, `defineComponent`, `register`, `hydrate`, plus convenience re-export of `html`/`render`)
* [~] No hidden behaviors behind imports — (core is fairly explicit; however direct re-export of `uhtml` exposes renderer internals)
...
* [ ] Maintain a **Security Model** document — (MISSING; recommend `docs/Security-Model.md`)
* [~] Explicitly state:

  * [x] what front.js does
  * [~] what it refuses to do — (largely covered but a dedicated Security Model doc would make this explicit)
  * [x] what risks it avoids
  * [~] what risks remain — (partially documented; add Security Model for clarity)
